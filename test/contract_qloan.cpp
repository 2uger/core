...
const id adminAddress = ID(_L, _O, _A, _N, _A, _A, _F, _B, _U, _G, _H, _E, _L, _A, _N, _X, _G, _H, _N, _L, _M, _S, _U, _I, _V, _B, _K, _B, _H, _A, _Y, _E, _Q, _S, _Q, _B, _V, _P, _V, _N, _B, _H, _L, _F, _J, _I, _A, _Z, _F, _Q, _C, _W, _W, _B, _V, _E);
...
class QLoanChecker : public QLOAN
{
public:
...
TEST(ContractQLoan, PlaceLoanReqValidation)
{
    ...
    qloan.placeLoanReq(testAddress1, assetName, testAddress1, 1000, 10, 10, 10, true, false, 2000);
    ...
    qloan.placeLoanReq(testAddress1, assetName, testAddress1, 300, 10, 10, 10, true, false, 3000);
...
TEST(ContractQLoan, PlaceLoanPrivateRequest)
{
    ...
    qloan.placeLoanReq(testAddress1, assetName, testAddress1, 300, 2000, 10, 10, true, false, 3000, testAddress3);
...
TEST(ContractQLoan, AcceptLoanReq)
{
    ...
    qloan.placeLoanReq(testAddress1, assetName, testAddress1, 300, 2000, 10, 10, true, false, 3000);
    ...
    EXPECT_EQ(fees.earnedAmount, 3030);
}
...
TEST(ContractQLoan, AcceptLoanReqCreditRequest)
{
    ...
    qloan.placeLoanReq(testAddress1, assetName, testAddress2, 300, 2000, 10, 10, false, false, 5000);
...
TEST(ContractQLoan, EpochDebtCalculation)
{
    ...
    std::string assetName = "ABCF";
    qloan.placeLoanReq(testAddress1, assetName, testAddress2, 300, 2000, 10, 2, false, false, 5000);
...
TEST(ContractQLoan, FeesDistribution)
{
    ...
    EXPECT_EQ(fees1.earnedAmount, 6000);
    EXPECT_EQ(fees1.distributedAmount, 0);
    EXPECT_EQ(fees1.toDevsAmount, 0);

    qloan.endEpoch();

    auto fees2 = qloan.getFeesInfo();
    EXPECT_EQ(fees2.distributedAmount, 5776);
    EXPECT_EQ(fees2.burnedAmount, 900);
    EXPECT_EQ(fees2.toDevsAmount, 3300);

    EXPECT_EQ(getBalance(adminAddress), 3300);
}
...
TEST(ContractQLoan, TotalUserDebt)
{
    ...
    qloan.placeLoanReq(testAddress1, assetName, testAddress2, 300, 2000, 10, 10, false, false, 5000);
    qloan.placeLoanReq(testAddress1, assetName, testAddress2, 300, 9000, 10, 10, false, false, 120000);

    qloan.issueAsset(testAddress2, assetName, 4000);
    qloan.transferRightsToQloan(testAddress2, assetName, testAddress2, 1000);

    auto reqs = qloan.getAllLoanReqs();
    qloan.acceptLoanReq(testAddress2, reqs.reqs.get(0).reqId, 1000);
    qloan.acceptLoanReq(testAddress2, reqs.reqs.get(1).reqId, 9000);
